...
class MPNN(nn.Module):
    """
    This function implements a Message Passing Neural Network (MPNN) layer for graph data.
    It applies a linear projection to node features, performs message passing using NNConv,
    updates node features using a GRU, and finally uses a Set2Set readout to get graph-level features.
...
class reactionMPNN(nn.Module):
   ...
   This function implements a reaction-specific Message Passing Neural Network (MPNN).
    It takes reactant and product molecular graphs as input, processes them using a shared MPNN layer,
    and concatenates the resulting graph-level features.
...
class encoder(nn.Module):
    ...
    def __init__(self, n_classes,
                 latent_feats, readout_feats, embedding_dim, predict_hidden_feats):
        ...
        self.predict = nn.Sequential(
            nn.Linear(readout_feats * 3 + n_classes + embedding_dim + 1, predict_hidden_feats), nn.PReLU(),
    ...
    def forward(self, vector, graph_embed=None, pos_neg_sample=1, embed=None):
        ...
        Args:
            ...
            embed (torch.Tensor, optional): Additional embeddings. Required for this model.
        ...
        if len(inputs_to_cat) == 1:
            warnings.warn("Encoder received only label vector. Ensure embeddings and pos_neg_sample are provided.")
...
class decoder_cat(nn.Module):  
    ...
    def __init__(self, rtype, n_info, latent_feats, readout_feats, embedding_dim, predict_hidden_feats):
        ...
        if rtype == 'bh':
            self.predict = nn.Sequential(
                nn.Linear(readout_feats * 3 + latent_feats + embedding_dim + 1, predict_hidden_feats), nn.PReLU(),
                nn.Linear(predict_hidden_feats, predict_hidden_feats), nn.PReLU(),
                nn.Linear(predict_hidden_feats, predict_hidden_feats), nn.PReLU(),
        ...
        if rtype == 'sm':
            self.predict = nn.Sequential(
                nn.Linear(n_sol_1 + n_sol_2 + 1 + n_add + readout_feats * 3 + latent_feats + embedding_dim + 1, predict_hidden_feats), nn.PReLU(),
                nn.Linear(predict_hidden_feats, predict_hidden_feats), nn.PReLU(),
                nn.Linear(predict_hidden_feats, predict_hidden_feats), nn.PReLU(),
                nn.Linear(predict_hidden_feats, n_cats)
            )

    def forward(self, input, pos_neg_sample=1, embed=None):
        if isinstance(pos_neg_sample, int):
            pos_neg_sample = torch.tensor([[pos_neg_sample]], device=input[0].device).expand(input[0].size(0), 1)

        if self.rtype == "bh":
            latent, graph_embed = input
            logits = self.predict(torch.cat([latent, graph_embed, embed, pos_neg_sample], 1))
            return logits
        if self.rtype == 'sm':
            latent, graph_embed, solv_1_pred, solv_2_pred, water_pred, add_pred = input
            logits = self.predict(torch.cat([latent, graph_embed, solv_1_pred, solv_2_pred, water_pred, add_pred, embed, pos_neg_sample], 1))
            return logits
...
class decoder_base(nn.Module):
    def __init__(self, rtype, n_info, latent_feats, readout_feats, embedding_dim, predict_hidden_feats):
        ...
        if rtype == 'bh':
            self.predict = nn.Sequential(
                nn.Linear(n_cats + readout_feats * 3 + latent_feats + embedding_dim + 1, predict_hidden_feats), nn.PReLU(),
                nn.Linear(predict_hidden_feats, predict_hidden_feats), nn.PReLU(),
                nn.Linear(predict_hidden_feats, predict_hidden_feats), nn.PReLU(),
        ...
        if rtype == 'sm':
            self.predict = nn.Sequential(
                nn.Linear(n_sol_1 + n_sol_2 + 1 + n_add + n_cats + readout_feats * 3 + latent_feats + embedding_dim + 1, predict_hidden_feats), nn.PReLU(),
                nn.Linear(predict_hidden_feats, predict_hidden_feats), nn.PReLU(),
                nn.Linear(predict_hidden_feats, predict_hidden_feats), nn.PReLU(),
                nn.Linear(predict_hidden_feats, n_base)
            )
    ...
    def forward(self, input, pos_neg_sample=1, embed=None):
        ...
        if self.rtype == "bh":
            ...
            logits = self.predict(torch.cat([latent, graph_embed, cat_pred, embed, pos_neg_sample], 1))
            return logits
        if self.rtype == 'sm':
            latent, graph_embed, solv_1_pred, solv_2_pred, water_pred, add_pred, cat_pred = input
            logits = self.predict(torch.cat([latent, graph_embed, solv_1_pred, solv_2_pred, water_pred, add_pred, cat_pred, embed, pos_neg_sample], 1))
            return logits
...
class decoder_solv_1(nn.Module):  
    def __init__(self, rtype, n_info, latent_feats, readout_feats, embedding_dim, predict_hidden_feats):
        ...
        if rtype == 'bh':
            self.predict = nn.Sequential(
                nn.Linear(n_base + n_cats + readout_feats * 3 + latent_feats + embedding_dim + 1, predict_hidden_feats), nn.PReLU(),
                nn.Linear(predict_hidden_feats, predict_hidden_feats), nn.PReLU(),
                nn.Linear(predict_hidden_feats, predict_hidden_feats), nn.PReLU(),
    ...
    def forward(self, input, pos_neg_sample=1, embed=None):
        ...
        if self.rtype == "bh":
            ...
            logits = self.predict(torch.cat([latent, graph_embed, cat_pred, base_pred, embed, pos_neg_sample], 1))
            return logits
        if self.rtype == 'sm':
            latent, graph_embed = input
            logits = self.predict(torch.cat([latent, graph_embed, embed, pos_neg_sample], 1))
...
class decoder_add(nn.Module):  
    def __init__(self, rtype, n_info, latent_feats, readout_feats, embedding_dim, predict_hidden_feats):
        ...
        if rtype == 'bh':
            self.predict = nn.Sequential(
                nn.Linear(n_base + n_cats + n_sol_1 + 1 + readout_feats * 3 + latent_feats + embedding_dim + 1, predict_hidden_feats), nn.PReLU(),
                nn.Linear(predict_hidden_feats, predict_hidden_feats), nn.PReLU(),
                nn.Linear(predict_hidden_feats, predict_hidden_feats), nn.PReLU(),
        ...
        if rtype == 'sm':
            self.predict = nn.Sequential(
                nn.Linear(n_sol_1 + n_sol_2 + 1 + readout_feats * 3 + latent_feats + embedding_dim + 1, predict_hidden_feats), nn.PReLU(),
                nn.Linear(predict_hidden_feats, predict_hidden_feats), nn.PReLU(),
                nn.Linear(predict_hidden_feats, predict_hidden_feats), nn.PReLU(),
                nn.Linear(predict_hidden_feats, n_add)
            )
    ...
    def forward(self, input, pos_neg_sample=1, embed=None):
        ...
        if self.rtype == "bh":
            ...
            logits = self.predict(torch.cat([latent, graph_embed, cat_pred, base_pred, solv_1_pred, water_pred, embed, pos_neg_sample], 1))
            return logits
        if self.rtype == 'sm':
            latent, graph_embed, sol_1_pred, sol_2_pred, water_pred = input
            logits = self.predict(torch.cat([latent, graph_embed, sol_1_pred, sol_2_pred, water_pred, embed, pos_neg_sample], 1))
            return logits
...
class decoder_water(nn.Module):  
    def __init__(self, rtype, n_info, latent_feats, readout_feats, embedding_dim, predict_hidden_feats):
        ...
        if rtype == 'bh':
            self.predict = nn.Sequential(
                nn.Linear(n_base + n_cats + n_sol_1 + readout_feats * 3 + latent_feats + embedding_dim + 1, predict_hidden_feats), nn.PReLU(),
                nn.Linear(predict_hidden_feats, predict_hidden_feats), nn.PReLU(),
                nn.Linear(predict_hidden_feats, predict_hidden_feats), nn.PReLU(),
        ...
        if rtype == 'sm':
            self.predict = nn.Sequential(
                nn.Linear(n_sol_1 + n_sol_2 + readout_feats * 3 + latent_feats + embedding_dim + 1, predict_hidden_feats), nn.PReLU(),
                nn.Linear(predict_hidden_feats, predict_hidden_feats), nn.PReLU(),
                nn.Linear(predict_hidden_feats, predict_hidden_feats), nn.PReLU(),
                nn.Linear(predict_hidden_feats, 1)
            )
    ...
    def forward(self, input, pos_neg_sample=1, embed=None):
        ...
        if self.rtype == "bh":
            ...
            logits = self.predict(torch.cat([latent, graph_embed, cat_pred, base_pred, solv_1_pred, embed, pos_neg_sample], 1))
            return logits  # Use sigmoid for binary classification
        if self.rtype == 'sm':
            latent, graph_embed, sol_1_pred, sol_2_pred = input
            logits = self.predict(torch.cat([latent, graph_embed, sol_1_pred, sol_2_pred, embed, pos_neg_sample], 1))
            return logits  # Use sigmoid for binary classification
...
class decoder_solv_2(nn.Module):  
    def __init__(self, rtype, n_info, latent_feats, readout_feats, embedding_dim, predict_hidden_feats):
        ...
        self.predict = nn.Sequential(
            nn.Linear(n_sol_1 + readout_feats * 3 + latent_feats + embedding_dim + 1, predict_hidden_feats), nn.PReLU(),
            nn.Linear(predict_hidden_feats, predict_hidden_feats), nn.PReLU(),
            nn.Linear(predict_hidden_feats, predict_hidden_feats), nn.PReLU(),
    ...
    def forward(self, input, pos_neg_sample=1, embed=None):
        ...
        logits = self.predict(torch.cat([latent, graph_embed, solv_1_pred, embed, pos_neg_sample], 1))
        return logits


class VAE_seq_emb(nn.Module):

    def __init__(self, rtype, node_in_feats, edge_in_feats,
                n_classes, n_info, embedding_dim=None,
                latent_feats = 128, readout_feats = 1024, predict_hidden_feats = 512):
        
        super(VAE_seq_emb, self).__init__()

        self.latent_feats = latent_feats
        self.rtype = rtype
        self.n_info = n_info  # Store n_info as instance variable

        self.rmpnn = reactionMPNN(node_in_feats, edge_in_feats, readout_feats)
        self.encoder = encoder(n_classes, latent_feats, readout_feats, embedding_dim, predict_hidden_feats)
        
        # Build decoders based on reaction type
        self.decoder_cat = decoder_cat(rtype, n_info, latent_feats, readout_feats, embedding_dim, predict_hidden_feats)
        self.decoder_base = decoder_base(rtype, n_info, latent_feats, readout_feats, embedding_dim, predict_hidden_feats)
        self.decoder_solv_1 = decoder_solv_1(rtype, n_info, latent_feats, readout_feats, embedding_dim, predict_hidden_feats)
        self.decoder_add = decoder_add(rtype, n_info, latent_feats, readout_feats, embedding_dim, predict_hidden_feats)
        self.decoder_water = decoder_water(rtype, n_info, latent_feats, readout_feats, embedding_dim, predict_hidden_feats)
        if rtype == 'sm':
            self.decoder_solv_2 = decoder_solv_2(rtype, n_info, latent_feats, readout_feats, embedding_dim, predict_hidden_feats)

    def forward(self, rmols=None, pmols=None, labels=None, pos_neg_sample=1, embed=None):
        """
        ...
        Args:
            ...
            labels (torch.Tensor): Input condition vector (labels).
            pos_neg_sample (int or torch.Tensor): Binary indicator for positive/negative sample. Defaults to 1.
            embed (torch.Tensor, optional): Additional embeddings. Required if using embeddings.

        Returns:
            tuple: (latent, graph_embed, mu, log_var) containing latent vector, graph embeddings, and distribution parameters.
        """
        # Assert that embeddings are provided
        assert embed is not None, "Embeddings are required for VAE_seq_emb model"
        assert isinstance(embed, torch.Tensor), "Embeddings must be a torch.Tensor"
        assert embed.dim() == 2, f"Embeddings must be 2D tensor, got shape {embed.shape}"
        assert embed.size(0) > 0, "Embeddings batch size must be greater than 0"

        graph_embed = self.rmpnn(rmols, pmols)
        
        # Encoding
        mu, log_var = self.encoder(labels, graph_embed, pos_neg_sample, embed)
    ...
    def sampling(self, rmols=None, pmols=None, pos_neg_sample=1, embed=None, config=None, temperature=1.0):
        """
        ...
        if self.rtype == 'bh':
            ...
            vector_pred_cat = self.decoder_cat([latent, graph_embed], pos_neg_sample, embed)
            if temperature != 1.0:
            ...
            vector_pred_base = self.decoder_base([latent, graph_embed, vector_pred_cat], pos_neg_sample, embed)
            if temperature != 1.0:
            ...
            vector_pred_solv_1 = self.decoder_solv_1([latent, graph_embed, vector_pred_cat, vector_pred_base], pos_neg_sample, embed)
            if temperature != 1.0:
            ...
            vector_pred_water = self.decoder_water([latent, graph_embed, vector_pred_cat, vector_pred_base, vector_pred_solv_1], pos_neg_sample, embed)
            if temperature != 1.0:
            ...
            vector_pred_add = self.decoder_add([latent, graph_embed, vector_pred_cat, vector_pred_base, vector_pred_solv_1, vector_pred_water], pos_neg_sample, embed)
            if temperature != 1.0:
            ...
            vector_pred_solv_1 = self.decoder_solv_1([latent, graph_embed], pos_neg_sample, embed)
            if temperature != 1.0:
                vector_pred_solv_1 = vector_pred_solv_1 / temperature
            prob_pred_solv_1 = torch.softmax(vector_pred_solv_1, dim=1)
            vector_pred_idx = torch.multinomial(prob_pred_solv_1, 1).squeeze(-1)
            ...
            vector_pred_solv_2 = self.decoder_solv_2([latent, graph_embed, vector_pred_solv_1], pos_neg_sample, embed)
            if temperature != 1.0:
            ...
            vector_pred_water = self.decoder_water([latent, graph_embed, vector_pred_solv_1, vector_pred_solv_2], pos_neg_sample, embed)
            if temperature != 1.0:
                vector_pred_water = vector_pred_water / temperature
            prob_pred_water = torch.sigmoid(vector_pred_water)
            vector_pred_water = (torch.bernoulli(prob_pred_water)).float()

            vector_pred_add = self.decoder_add([latent, graph_embed, vector_pred_solv_1, vector_pred_solv_2, vector_pred_water], pos_neg_sample, embed)
            if temperature != 1.0:
                vector_pred_add = vector_pred_add / temperature
            prob_pred_add = torch.sigmoid(vector_pred_add)
            vector_pred_add = (torch.bernoulli(prob_pred_add)).float()

            vector_pred_cat = self.decoder_cat([latent, graph_embed, vector_pred_solv_1, vector_pred_solv_2, vector_pred_water, vector_pred_add], pos_neg_sample, embed)
            if temperature != 1.0:
                vector_pred_cat = vector_pred_cat / temperature
            prob_pred_cat = torch.softmax(vector_pred_cat, dim=1)
            vector_pred_idx = torch.multinomial(prob_pred_cat, 1).squeeze(-1)
            ...
            vector_pred_base = self.decoder_base([latent, graph_embed, vector_pred_solv_1, vector_pred_solv_2, vector_pred_water, vector_pred_add, vector_pred_cat], pos_neg_sample, embed)
            if temperature != 1.0:
                vector_pred_base = vector_pred_base / temperature
            prob_pred_base = torch.softmax(vector_pred_base, dim=1)
            vector_pred_idx = torch.multinomial(prob_pred_base, 1).squeeze(-1)
...
class Trainer:
    ...
    def __init__(self, net, cuda, config, wandb_project="ReactionVAE_Seq_Emb"):
    ...
    def _get_losses_by_type(self, sample_type, loss_fns, latent, graph_embed, labels_tuple, pos_neg_sample, embed, epsilon):
        ...
        if self.net.rtype == 'bh':
            # --- Catalyst ---
            cat_logits = self.net.decoder_cat([latent, graph_embed], pos_neg_sample, embed)
            ...
            # --- Base ---
            base_logits = self.net.decoder_base([latent, graph_embed, next_cat_input], pos_neg_sample, embed)
            ...
            # --- Solvent 1 ---
            solv_1_logits = self.net.decoder_solv_1([latent, graph_embed, next_cat_input, next_base_input], pos_neg_sample, embed)
            ...
            # --- Water ---
            water_logits = self.net.decoder_water([latent, graph_embed, next_cat_input, next_base_input, next_solv_1_input], pos_neg_sample, embed)
            ...
            # --- Additive ---
            add_logits = self.net.decoder_add([latent, graph_embed, next_cat_input, next_base_input, next_solv_1_input, next_water_input], pos_neg_sample, embed)
            ...
            # --- Solvent 1 ---
            solv_1_logits = self.net.decoder_solv_1([latent, graph_embed], pos_neg_sample, embed)
            solv_1_logits = torch.clamp(solv_1_logits, -10, 10)
            all_losses['solv_1'] = select_loss('solv_1', 'CE')(solv_1_logits, labels_solv_1)
            next_solv_1_input = labels_solv_1_onehot if random.random() < epsilon else F.one_hot(torch.argmax(solv_1_logits.detach(), dim=1), num_classes=self.net.n_info[1]).float()
            
            # --- Solvent 2 ---
            solv_2_logits = self.net.decoder_solv_2([latent, graph_embed, next_solv_1_input], pos_neg_sample, embed)
            ...
            # --- Water ---
            water_logits = self.net.decoder_water([latent, graph_embed, next_solv_1_input, next_solv_2_input], pos_neg_sample, embed)
            water_logits = torch.clamp(water_logits, -10, 10)
            all_losses['water'] = select_loss('water', 'BCE')(water_logits, labels_water).sum(axis=1).mean()
            next_water_input = labels_water if random.random() < epsilon else (torch.sigmoid(water_logits.detach()) >= 0.5).float()

            # --- Additive ---
            add_logits = self.net.decoder_add([latent, graph_embed, next_solv_1_input, next_solv_2_input, next_water_input], pos_neg_sample, embed)
            add_logits = torch.clamp(add_logits, -10, 10)
            all_losses['add'] = select_loss('add', 'BCE')(add_logits, labels_add).sum(axis=1).mean()
            next_add_input = labels_add if random.random() < epsilon else (torch.sigmoid(add_logits.detach()) >= 0.5).float()

            # --- Catalyst ---
            cat_logits = self.net.decoder_cat([latent, graph_embed, next_solv_1_input, next_solv_2_input, next_water_input, next_add_input], pos_neg_sample, embed)
            cat_logits = torch.clamp(cat_logits, -10, 10)
            all_losses['cat'] = select_loss('cat', 'CE')(cat_logits, labels_cat)
            next_cat_input = labels_cat_onehot if random.random() < epsilon else F.one_hot(torch.argmax(cat_logits.detach(), dim=1), num_classes=self.net.n_info[0]).float()

            # --- Base ---
            base_logits = self.net.decoder_base([latent, graph_embed, next_solv_1_input, next_solv_2_input, next_water_input, next_add_input, next_cat_input], pos_neg_sample, embed)
            base_logits = torch.clamp(base_logits, -10, 10)
            all_losses['base'] = select_loss('base', 'CE')(base_logits, labels_base)
    ...
    def training(self, train_loader, val_loader, max_epochs = 250, calc_diversity = True):
        """
        Training method for the VAE_seq_emb model.
        Combines sequential prediction with embedding-based approaches.
        Now includes scheduled sampling and improved loss computation.
        """
        loss_fns = {}
        ...
        for epoch in range(max_epochs):
            ...
            for batchidx, batchdata in enumerate(train_loader):
                ...
                labels = batchdata[-3]  # Labels are third to last
                pos_neg_sample = batchdata[-2].to(self.cuda)  # pos_neg_sample is second to last
                embed = batchdata[-1].to(self.cuda)  # Embeddings are last

                labels_tuple = [t.to(self.cuda) for t in labels]
                latent, graph_embed, mu, log_var = self.net(inputs_rmol, inputs_pmol, labels_tuple[0], pos_neg_sample, embed)
                ...
                def process_sub_batch(mask, sample_type):
                    ...
                    sub_batch_losses_dict = self._get_losses_by_type(sample_type, loss_fns, latent[mask], graph_embed[mask], sub_batch_labels, pos_neg_sample[mask], embed[mask], epsilon)
    ...
    def inference(self, tst_loader, n_sampling = 100, temperature=1.0):
        ...
        def forward(tst_loader, pos_neg_flag=1, config=self.config):
            ...
            with torch.no_grad():
                for batchidx, batchdata in enumerate(tst_loader):
                    if not batchdata:
                        print("WARNING: Empty batch detected in VAE_seq_emb inference. Skipping.")
                        continue
                    inputs_rmol = [b.to(self.cuda) for b in batchdata[:2]]
                    inputs_pmol = [b.to(self.cuda) for b in batchdata[2:3]]
                    embed = batchdata[-1].to(self.cuda)  # embeddings are last
                    ...
                    preds = self.net.sampling(inputs_rmol, inputs_pmol, pos_neg_sample, embed, config=config, temperature=temperature)
            ...
            if not tst_y_preds:
                print("WARNING: No predictions generated in VAE_seq_emb inference. Returning empty list.")
                return []
            return tst_y_preds
        ...
        for _ in range(n_sampling):
            ...
            if not batch_preds:
                print("WARNING: No positive predictions were made in a sampling run.")
                continue  # Or break if one failure is enough
            if not tst_y_preds_pos:  # First sampling
                tst_y_preds_pos = [[pred] for pred in batch_preds]
            else:  # Subsequent samplings
                if len(tst_y_preds_pos) != len(batch_preds):
                    print(f"WARNING: Batch size mismatch during positive sampling. Expected {len(tst_y_preds_pos)}, got {len(batch_preds)}. Skipping update.")
                    continue
        ...
        for _ in range(n_sampling):
            ...
            if not batch_preds:
                print("WARNING: No negative predictions were made in a sampling run.")
                continue # Or break
            if not tst_y_preds_neg:  # First sampling
                tst_y_preds_neg = [[pred] for pred in batch_preds]
            else:  # Subsequent samplings
                if len(tst_y_preds_neg) != len(batch_preds):
                    print(f"WARNING: Batch size mismatch during negative sampling. Expected {len(tst_y_preds_neg)}, got {len(batch_preds)}. Skipping update.")
                    continue
                for i, pred in enumerate(batch_preds):
