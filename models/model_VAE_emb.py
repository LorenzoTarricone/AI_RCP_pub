...
# use_wandb = False #used for easily turn on and off wandb tracking

class MPNN(nn.Module):
    """Message Passing Neural Network layer for graphs."""
    ...
    def forward(self, g):
        ...
        for _ in range(self.num_step_message_passing):
            ...
            #TODO: check if batch_first needs to be set to True
...
class encoder(nn.Module):
    """Encoder part of the VAE, mapping input + context to latent space."""
    def __init__(self, n_classes, latent_feats, gnn_readout_feats, embedding_dim, predict_hidden_feats):
        super(encoder, self).__init__()
        self.latent_feats = latent_feats

        # Calculate total input dimension based on available embeddings
        encoder_input_dim = n_classes # Labels are always present
        encoder_input_dim += 1 #Always one binary number indicating positive or negative sample
        encoder_input_dim += gnn_readout_feats * 3  # 2 reactants + 1 product concatenated
        encoder_input_dim += embedding_dim # Already doubled in dataset

        print(f"Encoder input dimension: {encoder_input_dim}")

        # Define the prediction network
        self.predict = nn.Sequential(
            nn.Linear(encoder_input_dim, predict_hidden_feats), nn.PReLU(),
            nn.Linear(predict_hidden_feats, predict_hidden_feats), nn.PReLU(),
            nn.Linear(predict_hidden_feats, predict_hidden_feats), nn.PReLU(),
            nn.Linear(predict_hidden_feats, latent_feats * 2) # Output mu and log_var
        )
    ...
    def forward(self, vector, graph_embed=None, pos_neg_sample=1, embed=None):
        ...
        Args:
            ...
            embed (torch.Tensor, optional): Additional embeddings. Defaults to None.
        ...
        if len(inputs_to_cat) != 4:
            warnings.warn("Encoder received wrong number of inputs. Ensure label, graph_embed, pos_neg_sample and embed are provided.")
        ...
        # Clamp log_var to prevent numerical instability in KLD loss
...
# --- MODIFIED: decoder Class ---
class decoder(nn.Module):
    """Decoder part of the VAE, mapping latent space + context back to input space."""
    def __init__(self, n_classes, latent_feats, gnn_readout_feats, embedding_dim, predict_hidden_feats):
        super(decoder, self).__init__()

        # Calculate total input dimension based on available embeddings + latent vector
        decoder_input_dim = latent_feats # Latent vector is always present
        decoder_input_dim += 1 #Always one binary number indicating positive or negative sample
        if gnn_readout_feats is not None:
            decoder_input_dim += gnn_readout_feats * 3  # 2 reactants + 1 product concatenated
        if embedding_dim is not None:
            decoder_input_dim += embedding_dim # Already doubled in dataset

        print(f"Decoder input dimension: {decoder_input_dim}")

        # Define the prediction network
        self.predict = nn.Sequential(
            nn.Linear(decoder_input_dim, predict_hidden_feats), nn.PReLU(),
            nn.Linear(predict_hidden_feats, predict_hidden_feats), nn.PReLU(),
            nn.Linear(predict_hidden_feats, predict_hidden_feats), nn.PReLU(),
            nn.Linear(predict_hidden_feats, n_classes) # Output logits for condition classes
        )

    def forward(self, latent, graph_embed=None, pos_neg_sample=1, embed=None):
        """
        Forward pass for the decoder.

        Args:
            latent (torch.Tensor): Latent space vector.
            graph_embed (torch.Tensor, optional): Concatenated GNN embeddings. Defaults to None.
            pos_neg_sample (int or torch.Tensor): Binary indicator for positive/negative sample. Defaults to 1.
            embed (torch.Tensor, optional): Concatenated embeddings. Defaults to None.

        Returns:
            torch.Tensor: Predicted logits for the condition classes.
        """
         # Build input by concatenating available features
        inputs_to_cat = [latent]
        ...
        if len(inputs_to_cat) != 4:
            warnings.warn("Decoder received wrong number of inputs. Ensure latent, graph_embed, pos_neg_sample and embed are provided.")

        combined_input = torch.cat(inputs_to_cat, 1)

        # Predict condition logits
        return self.predict(combined_input)


# --- MODIFIED: VAE_emb Class (Main Model) ---
# Renamed from VAE to VAE_emb
class VAE_emb(nn.Module):
    """Variational Autoencoder model integrating GNN and/or ChemBERTa embeddings."""
    def __init__(self, node_in_feats, edge_in_feats, n_classes,
                 embedding_dim=None, # Added ChemBERTa dimension
                 latent_feats = 128, gnn_readout_feats = 1024, predict_hidden_feats = 512):
        """
        Initializes the VAE_emb model.

        Args:
            node_in_feats (int): Dimension of node features (if graphs used).
            edge_in_feats (int): Dimension of edge features (if graphs used).
            n_classes (int): Number of output condition classes.
            embedding_dim (int, optional): Dimension of concatenated (r+p) ChemBERTa embeddings.
                                                    Set to None if not using ChemBERTa. Defaults to None.
            latent_feats (int): Dimension of the latent space. Defaults to 128.
            gnn_readout_feats (int): Output dimension of the MPNN readout layer. Defaults to 1024.
            predict_hidden_feats (int): Hidden dimension size for encoder/decoder MLPs. Defaults to 512.
        """
        super(VAE_emb, self).__init__()

        self.latent_feats = latent_feats
        self.gnn_readout_feats = gnn_readout_feats
        self.embedding_dim = embedding_dim

        # Initialize GNN component only if graph dimensions are provided
        self.rmpnn = reactionMPNN(node_in_feats, edge_in_feats, gnn_readout_feats)


        # Initialize encoder and decoder with potentially None dimensions
        self.encoder = encoder(n_classes, latent_feats, self.gnn_readout_feats, self.embedding_dim, predict_hidden_feats)
        self.decoder = decoder(n_classes, latent_feats, self.gnn_readout_feats, self.embedding_dim, predict_hidden_feats)
    ...
    def forward(self, rmols=None, pmols=None, labels=None, pos_neg_sample=1, embed=None):
        ...
        Args:
            rmols (list, optional): List of reactant DGL graphs (or None). Required if using graphs.
            pmols (list, optional): List of product DGL graphs (or None). Required if using graphs.
            labels (torch.Tensor): Input condition labels (target).
            embed (torch.Tensor, optional): Pre-computed ChemBERTa embeddings. Required if using ChemBERTa.

        Returns:
            tuple: (vector_pred, mu, log_var) containing predicted logits, latent mean, and latent log variance.
        """
        graph_embed = None
        # 1. Get GNN embeddings if graphs are used and provided
        if rmols is None or pmols is None or self.rmpnn is None:
            raise ValueError("Reactant/Product graphs (rmols, pmols) must be provided when using graphs.")
        graph_embed = self.rmpnn(rmols, pmols) # Shape: [batch, gnn_readout_feats * 2]
        ...
        # 5. Decoder step
        # Pass available embeddings
        vector_pred = self.decoder(latent, graph_embed, pos_neg_sample, embed)
    ...
    def sampling(self, rmols=None, pmols=None, pos_neg_sample=1, embed=None, batch_size=None, device=None, temperature=1.0):
        """
        ...
        # 1. Get GNN embeddings if graphs are used
        if rmols is None or pmols is None or self.rmpnn is None:
            raise ValueError("Reactant/Product graphs (rmols, pmols) must be provided when using graphs for sampling.")
        graph_embed = self.rmpnn(rmols, pmols)
        current_batch_size = graph_embed.shape[0]
        current_device = graph_embed.device

        # 2. Use embeddings if provided
        if embed is None:
            raise ValueError("Embeddings must be provided when use_embeddings=True for sampling.")
        if current_batch_size is None:
            current_batch_size = embed.shape[0]
            current_device = embed.device
        elif current_batch_size != embed.shape[0]:
            warnings.warn(f"Batch size mismatch between graph ({current_batch_size}) and ChemBERTa ({embed.shape[0]}) embeddings during sampling.")
            current_batch_size = embed.shape[0]


        # 3. Sample latent vector
        if current_batch_size is None or current_device is None:
             raise ValueError("Cannot determine batch size or device for latent vector sampling. Provide embeddings or batch_size/device.")
...
class Trainer:
    ...
    def __init__(self, net, cuda, config,  wandb_project="ReactionVAE_embedd"):
        ...
        if config["wandb"]:
            if wandb.run is None:
                wandb.init(project=self.wandb_project, config={
                    ...
                    "gnn_readout_feats": net.gnn_readout_feats,
                    "embedding_dim": net.embedding_dim,
                    "predict_hidden_feats": net.encoder.predict[0].out_features, # Infer from net
                })
    ...
    def training(self, train_loader, val_loader, max_epochs = 250, calc_diversity = True):
        ...
        for epoch in range(max_epochs):
            ...
            for batchidx, batchdata in enumerate(train_loader):
                ...
                embedd = batchdata[-1].to(self.cuda)
                pos_neg_sample = batchdata[-2].to(self.cuda)
                labels = batchdata[-3].to(self.cuda)

                preds, mu, log_var = self.net(inputs_rmol, inputs_pmol, labels, pos_neg_sample, embedd)
            ...
            print('\n--- training epoch %d, lr %f, loss %.3f, bce_loss %.3f, kld_loss %.3f, time elapsed(min) %.2f, max gradient norm %.3f'
            ...
            # --- Sometimes val_y_pos can have an empty sublist (i.e. for that product we just have negative reactions) ---
            # ---------- we want to correct for it by not considering the predictions of our model in that case -----------
            ...
            if filtered_pos_pairs:
                ...
                # If all were removed, result is two empty lists
            ...
            micro_recall_pos = np.sum([np.sum([(c in filtered_val_y_preds_pos[i]) for c in filtered_val_y_pos[i]]) for i in range(len(filtered_val_y_pos))]) / np.sum([len(a) for a in filtered_val_y_pos]) if filtered_val_y_pos else 0.0
            ...
            if self.config["data_type"] == "all":
                ...
                micro_recall_neg = np.sum([np.sum([(c in filtered_val_y_preds_neg[i]) for c in filtered_val_y_neg[i]]) for i in range(len(filtered_val_y_neg))]) / np.sum([len(a) for a in filtered_val_y_neg]) if filtered_val_y_neg else 0.0
           
           
            #TODO: think aboout the fact that  in the OG version, these are not the same scale!! You will be basically update the model according to the accuracy
            # val_loss = 1 - (accuracy + macro_recall + micro_recall) / 3 #OG
            if self.config["data_type"] == "all":
    ...
    def inference(self, tst_loader, n_sampling = 100, temperature=1.0):
        ...
        def forward(tst_loader, pos_neg_flag=1):
            ...
            with torch.no_grad():
                for batchidx, batchdata in enumerate(tst_loader):
                    ...
                    pos_neg_sample = torch.ones((embedd.shape[0],1)).to(self.cuda) if pos_neg_flag == 1 else torch.zeros((embedd.shape[0],1)).to(self.cuda)

                    preds_list = self.net.sampling(inputs_rmol, inputs_pmol, pos_neg_sample, embedd, temperature=temperature).cpu().numpy() #produces logits for n_classes
        ...
        self.net.eval()   
        tst_y_preds_pos_list = [forward(tst_loader, 1) for _ in range(n_sampling)]
        ...
        tst_y_preds_neg_list = [forward(tst_loader, 0) for _ in range(n_sampling)]
